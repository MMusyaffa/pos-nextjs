...
export default function ProductsCart() {
    ...
    return (
        <>
            <a href="#" className="flex flex-col max-h-90 space-y-4 overflow-auto">
                {
                    cart.map((productInCart) => 
                        <a key={productInCart.id} onMouseLeave={() => setCurrentIdProduct(null)} onClick={() => setCurrentIdProduct(productInCart.id)} className={`flex flex-none bg-stone-700 rounded-md relative p-2 h-14 text-xs overflow-hidden transition-all duration-200 ease-in-out hover:opacity-80 }`}>
