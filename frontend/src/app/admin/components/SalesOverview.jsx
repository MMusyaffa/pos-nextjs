...
export default function SalesOverview() {
    ...
    useEffect(() => {
        const fetchSalesData = async () => {
            try {
                const salesData = await getSalesData(); // Fetch sales data from API

                const filteredData = salesData.filter(item => {
                    const itemDate = item.date.toString(); // Convert date to string
                    const itemYear = parseInt(itemDate.substring(0, 4), 10);  // Get year
                    const itemMonth = parseInt(itemDate.substring(4, 6), 10); // Get month
                ...
                setCategories(filteredData.map(item => {
                    ...
                    const day = itemDate.substring(6, 8);  // Get day from YYYYMMDD format
    ...
    return (
        <div className="card shadow-md bg-base-100 ml-56 mt-10">
